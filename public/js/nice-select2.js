  ...
  (() => {
    ...
    return (
      ...
      (u.prototype.renderDropdown = function () {
        ...
        let t = '<div className="nice-select-search-box">';
        (t += `<input type="text" className="nice-select-search" placeholder="${this.searchtext}..." title="search">`),
          (t += "</div>");
        var i = `<div className="${e.join(" ")}" tabIndex="${
          this.disabled ? null : 0
        }">`;
        (i += `<span className="${
          this.multiple ? "multiple-options" : "current"
        }"></span>`),
          (i += '<div className="nice-select-dropdown">'),
          (i += `${this.config.searchable ? t : ""}`),
          (i += '<ul className="list"></ul>'),
          (i += "</div>"),
          (i += "</div>"),
        ...
        if (this.multiple) {
          ...
          this.selectedOptions.length < 2
            ? (this.selectedOptions.forEach(function (t) {
                e += `<span className="current">${t.data.text}</span>`;
              }),
